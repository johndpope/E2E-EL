    ...
    Examples::
        ...
        if mode == 'train':
            ...
            for k in mention_span_token_embeddings:
                ...
                idx_positive_candidate = entities_to_idx[target_entity]
                p_c_embedding = self.static_embedding(torch.LongTensor([idx_positive_candidate]).cuda())

                # Get the embeddings of the negative candidates
                negative_candidate_embeddings = []
                for c in entities[target_entity]['candidates']:
                    if c != target_entity:
                        idx_negative_candidate = entities_to_idx[c]
                        n_c_embedding = self.static_embedding(torch.LongTensor([idx_negative_candidate]).cuda())
            ...
            for k in mention_span_token_embeddings:
                ...
                candidate_scores = {}
                for c in entities[target_entity]['candidates']:
                    idx_candidate = entities_to_idx[c]
                    c_embedding = self.static_embedding(torch.LongTensor([idx_candidate]).cuda())
