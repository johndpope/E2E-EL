...
def get_marked_mentions(document_id, mentions, docs,  max_seq_length, tokenizer):
    ...
    for m in mentions[document_id]:
        ...
        # The sequence tag for prefix tokens is 'O'
        for j, _ in enumerate(prefix_tokens):
            sequence_tags.append('O')
        ...
        for j, _ in enumerate(mention_tokens):
            sequence_tags.append('B' if j == 0 else 'I')
    ...
    if len(suffix) > 0:
        ...
        for j, _ in enumerate(suffix_tokens):
            sequence_tags.append('O')
