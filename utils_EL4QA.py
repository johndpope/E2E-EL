...
def get_examples(data_dir, mode):
    ...
    for file in ['train.jsonl', 'test.jsonl', 'dev.jsonl']:
        ...
        with open(file_path) as f:
            for line in f:
                ...
                label_id = fields["label_id"] # Wikipedia ID
                entity = fields["entity"]
                label = fields["label"]  # This is the entity description

                for i, l_id in enumerate(label_id):
                    if l_id not in entities:
                        entities[l_id] = entity[i] + ' [SEP]' + label[i]
    ...
    if mode is 'train':
        ...
        file_path = os.path.join(data_dir, 'test.jsonl')
    with open(file_path, encoding='utf-8') as f:
        print("{} dataset is loading.... .".format(mode))
        for line in f:
            fields = json.loads(line.strip())
            docs[fields["id"]] = {"text": fields["text"]}
            mention_boundaries = fields["mentions"]
            ments[fields["id"]] = []
            for i, m in enumerate(mention_boundaries):
                start_index = m[0]
                end_index = m[1]
                content_document_id = fields["id"]
                label_candidate_id = fields["label_id"][i]
                text = fields["entity"][i]
                ments[fields["id"]].append({"mention_id": str(content_document_id) + '-' + str(i+1),
                                            "start_index": start_index,
                                            "end_index": end_index,
                                            "content_document_id": content_document_id,
                                            "label_candidate_id": label_candidate_id,
                                            "text": text})
        print("{} Loading dataset is done :)".format(mode))
...
def convert_examples_to_features(
    ...
    for c_idx, c in enumerate(all_entities):
        ...
        max_entity_len = 128  # Number of tokens
    ...
    for (ex_index, document_id) in enumerate(mentions.keys()):
        ...
        if args.do_train:
            if args.use_random_candidates:  # Random negatives
                ...
                all_candidate_embeddings_ = all_candidate_embeddings.unsqueeze(0).expand(num_m, -1,
                                                                                         hidden_size)  # M X C_all X H
                ...
                for m_idx, m in enumerate(mentions[document_id]):
                    ...
                    if len(mention_hard_negatives[mention_id]) < args.num_candidates:  # args.num_candidates - 1
                        ...
                        m_hard_candidates = random.sample(candidate_pool,
                                                          args.num_candidates)  # args.num_candidates - 1
        ...
        if args.use_all_candidates:
            ...
            candidate_token_ids_1 = [[tokenizer.pad_token_id] * max_entity_len] * (
    ...
    all_document_ids, all_label_candidate_ids)
